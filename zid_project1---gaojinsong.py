...
# ----------------------------------------------------------------------------
ROOTDIR = os.path.join(cfg.PRJOIR,'project1_3')
...
#
COLWIDTHS = {'Volume':14,'Date':11,'Adj Close':19,'Close':10,'Open':6,'High':6}
...
def get_tics(pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth) as apple_in:
        tics= {}
        for line in apple_in:
            tics[line.split('"')[3].lower()]=line.split('"')[1].lower()
    return tics
...
def read_dat(tic):
    ...
    with open(tic_path) as apple_in
        lines = []
        for moon in apple_in:
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for i in tickers_lst:
        data = []
        for j in range(len(read_dat(i))):
            dict_2 = line_to_dict(read_dat(i)[j])
            data.append(dict([(key,dict_2[key]) for key in col_lst]))
        dict_1[i]={
                    'exchange': tic_exchange_dic[i],
                    'data': data
                }
    return dict_1
